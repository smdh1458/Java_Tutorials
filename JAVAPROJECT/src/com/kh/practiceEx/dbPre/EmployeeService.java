...
public interface EmployeeService {
     ...
     * 직원 정보 수정 기능 (직원 정보를 수정할 때 사원ID, 사원주민등록번호, 입사일은 변경 불가)
     * @param emp_name
     * @param email
     * @param phone
     * @param dept_code
     ...
     * @param emt_yn
     */
    void updateEmployee(String emp_name, String email, String phone, String dept_code,String job_code,String sal_level,double salary, double bonus,String manager_id, Date ent_date,  char emt_yn);

    /** set을 이용해서 사원번호부터 퇴사여부까지 14가지 항목을 모두 전달받는 것이 아니라
    14가지 컬럼중에서 4가지 컬럼이 일치하는 사원을 삭제
    ...
    * 일치 여부에 따른 값을 작성하고 작성한 내용이 DB에 일치하다면 회원정보 조회가능
    ...
    * @param emp_name
    ...
    /**
     * 특정 회원을 찾고자하는 상위 직급을 위한 회원정보 조회 가능
     * 회원정보 조회 기능
     * 회원정보를 조회할 때 사원번호가 존재하면 조회 가능
     * @param info_emp
     * 사원번호 또는 사원이름 또는 사원 이메일 중에 하나라도 일치하는 정보가 존재한다면 회원정보 조회가능
     */
    ...
    /**
     * 사원이 로그인을 진행할 떄 이름과 핸드폰번호 일치여부 로그인 진행
     * @param emp_name
     * @param phone
     */
